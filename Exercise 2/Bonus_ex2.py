...
def dt_step(pedestrian_number, dist, env, grid_size, X, target, speed, dt):
    ...
    for ped in range(pedestrian_number):
        ...
        # Divide in direction by the norm of the distance and multiply by the speed 
        X[ped][0] += move*(Y[ped][0]-X[ped][0])/norm
        X[ped][1] += move*(Y[ped][1]-X[ped][1])/norm
    return X
